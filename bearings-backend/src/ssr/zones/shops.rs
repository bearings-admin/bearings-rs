...
fn store_card(st: &StoreRow) -> String {
    ...
    if st.bear_owned.unwrap_or(false)     { badges.push(badge("bear-owned", GOLD, DARK)); }
    if st.size_inclusive.unwrap_or(false) { badges.push(badge("size incl.", TAN, BROWN)); }
    ...
    let desc_h = if sdesc.is_empty() { String::new() } else {
        format!("<div style=\"font-size:12px;color:{MID};margin-top:3px;line-height:1.5\">{}</div>", sdesc.chars().take(140).collect::<String>())
    };
    let content = format!(
...
pub(crate) async fn zone_shops(db: SupabaseClient, lang: &str) -> Response {
    ...
    if !owned.is_empty() {
        ...
        for &s in &owned { body.push_str(&store_card(s)); }
    }
    if !other.is_empty() {
        body.push_str(&sh("More gear & marketplaces", Some(other.len())));
        for &s in &other { body.push_str(&store_card(s)); }
    }
    Html(shell("Bear Shops & Gear", "Bear-owned shops and community gear.", "shops", &body, lang)).into_response()
}
