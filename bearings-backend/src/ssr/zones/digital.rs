...
fn digital_card(s: &DigitalSpaceRow, events_html: &str) -> String {
    ...
    let mut sc = Vec::new();
    if !ig.is_empty() { sc.push(link_badge(&format!("https://instagram.com/{ig}"), "IG", "#E1306C")); }
    if !tt.is_empty() { sc.push(link_badge(&format!("https://tiktok.com/@{tt}"), "TikTok", "#000")); }
    if !bs.is_empty() { sc.push(link_badge(&format!("https://bsky.app/profile/{bs}"), "Bluesky", "#0085ff")); }
    if !yt.is_empty() { sc.push(link_badge(&format!("https://youtube.com/{yt}"), "YouTube", "#FF0000")); }

    let region = s.country.as_deref().filter(|c| !c.is_empty())
        .map(|c| badge(&format!("\u{1f4cd} {c}"), TAN, BROWN)).unwrap_or_default();
    let m_html = s.member_count.map(|m| badge(&format!("{m} members"), TAN, BROWN)).unwrap_or_default();
    ...
    let desc_h = if desc.is_empty() { String::new() } else {
        format!("<div style=\"font-size:12px;color:{MID};margin-top:3px;line-height:1.5\">{}</div>", desc.chars().take(150).collect::<String>())
    };
    let sc_block = if sc.is_empty() { String::new() } else {
        format!("<div style=\"margin-top:6px;display:flex;gap:4px;flex-wrap:wrap\">{}</div>", sc.join(""))
    };
...
pub(crate) async fn zone_digital(db: SupabaseClient, lang: &str) -> Response {
    ...
    struct EvLite { name: String, start_date: Option<String> }
    #[derive(serde::Deserialize)]
    struct Link { digital_space_id: i64, relationship: Option<String>, events: Option<EvLite> }
    ...
    let links: Vec<Link> = db.get_json::<Vec<Link>>(&links_url).await.or_log("digital:links");
    ...
    for l in &links {
        if let Some(ev) = &l.events {
            if !seen.entry(l.digital_space_id).or_default().insert(ev.name.clone()) { continue; }
            let date = ev.start_date.as_deref().map(|d| format!(" \u{00b7} {}", esc(d))).unwrap_or_default();
            let rel  = esc(l.relationship.as_deref().unwrap_or("hosts"));
            ev_map.entry(l.digital_space_id).or_default()
                .push(format!("<div>{} {}{}</div>", esc(&ev.name), rel_paren(&rel), date));
        }
    }
    ...
    const BUCKETS: [&str; 5] = [
        "Apps to meet bears", "Communities & servers", "Watch & listen",
        "Bear media & press", "More online spaces",
    ];
    ...
    for (i, label) in BUCKETS.iter().enumerate() {
        let group: Vec<&DigitalSpaceRow> = spaces.iter()
            .filter(|s| bucket_of(s.space_type.as_deref().unwrap_or("")) == i).collect();
    ...
    Html(shell("Digital Spaces", "Where the bear community gathers online.", "now", &body, lang)).into_response()
}

/// Render a relationship as " (organiser)" unless it is the default "hosts".
fn rel_paren(rel: &str) -> String {
    if rel == "hosts" { String::new() } else { format!(" ({rel})") }
}
