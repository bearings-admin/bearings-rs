...
pub async fn coming_up_page(State(db): State<SupabaseClient>) -> Response {
    zone_coming_up(db, None, None, None, "en").await
}

pub async fn history_page(State(db): State<SupabaseClient>) -> Response {
    zone_archive(db, None, None, "en").await
}
pub async fn bear_future_page(State(db): State<SupabaseClient>) -> Response {
    zone_future(db, "en").await
}
pub async fn events_page(State(db): State<SupabaseClient>) -> Response {
    zone_events(db, None, "en").await
}
pub async fn places_page(State(db): State<SupabaseClient>) -> Response {
    zone_places(db, None, None, "en").await
}
pub async fn clubs_page(State(db): State<SupabaseClient>) -> Response {
    zone_clubs(db, "en").await
}
pub async fn titles_page(State(db): State<SupabaseClient>) -> Response {
    zone_titles(db, "en").await
}
pub async fn creators_page(State(db): State<SupabaseClient>) -> Response {
    zone_creators(db, "en").await
}
pub async fn campaigns_page(State(db): State<SupabaseClient>) -> Response {
    zone_campaigns(db, "en").await
}
pub async fn digital_spaces_page(State(db): State<SupabaseClient>) -> Response {
    zone_digital(db, "en").await
}
    ...
    fn zone_parse_all_known() {
        ...
        for (input, expected) in cases {
            assert_eq!(
                Zone::parse(input),
                expected,
                "?zone={input} did not parse correctly"
            );
        }
    }
