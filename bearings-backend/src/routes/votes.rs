...
pub async fn cast(
    ...
    let outcome = service
        .cast(req.proposal_id, req.voter_id, &req.vote)
        .await?;
    map_outcome(outcome)
}
...
fn map_outcome(outcome: VoteOutcome) -> Result<(StatusCode, Json<VoteResponse>), AppError> {
    ...
    match outcome {
        VoteOutcome::InvalidVote => resp(
            ...
            "rejected",
            "Vote must be 'yes', 'no', or 'abstain'.".into(),
            None,
            None,
            None,
        ),
        ...
        VoteOutcome::ProposalNotFound { proposal_id } => Err(AppError::NotFound(format!(
        ...
        VoteOutcome::Recorded {
            ...
            resp(
                ...
                "recorded",
                message,
                Some(if passed { "passed" } else { "open" }.into()),
                Some(yes),
                Some(no),
            )
        }
