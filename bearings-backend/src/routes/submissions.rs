...
pub async fn create(
    ...
    if payload.name.trim().is_empty() {
        return Ok((StatusCode::UNPROCESSABLE_ENTITY, Json(SubmissionResponse {
    ...
    if contains_injection(&payload.name) || payload.description.as_deref().map(contains_injection).unwrap_or(false) {
        ...
        return Ok((StatusCode::OK, Json(SubmissionResponse {
            status: "pending_review".to_string(),
            message: "Your submission is under review. You'll receive an email when it's live.".to_string(),
            submission_id: None,
        })));
    }
    ...
    let submission_id = write_submission(&db, &payload, "pending_review", privacy_mode, false).await?;

    Ok((StatusCode::CREATED, Json(SubmissionResponse {
        status: "pending_review".to_string(),
        message: "Thanks! Your submission is under review. We'll email you when it's live.".to_string(),
...
async fn write_submission(
    ...
    let resp: Vec<serde_json::Value> = db.write_json_returning(
...
fn contains_injection(text: &str) -> bool {
    ...
    let patterns = [
        ...
        "act as", "roleplay as", "pretend you",
...
async fn is_duplicate(
    ...
    if !results.is_empty() { return Ok(true); }
