...
//!
//! HTTP-only: data access lives in `repositories::revival_repo`.

use crate::db::SupabaseClient;
use crate::repositories::revival_repo::{RevivalRepository, SupabaseRevivalRepository};
use axum::{
    extract::{Path, State},
    http::StatusCode,
...
pub async fn vote(
    ...
    let repo = SupabaseRevivalRepository::new(db);
    match repo.record_vote(&kind, id).await {
        Ok(Some(c)) => Html(format!(
        ...
        .into_response(),
        Ok(None) => (StatusCode::NOT_FOUND, Html("not found".to_string())).into_response(),
        Err(_) => {
