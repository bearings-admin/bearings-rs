...
pub struct SupabaseCreatorRepository { db: SupabaseClient }
impl SupabaseCreatorRepository { pub fn new(db: SupabaseClient) -> Self { Self { db } } }
...
impl CreatorRepository for SupabaseCreatorRepository {
    async fn find(&self, filter: CreatorFilter) -> Result<Vec<Creator>, AppError> {
        ...
        if let Some(t) = filter.creator_type { url.push_str(&clause("creator_type", "eq", &t)); }
        if let Some(c) = filter.country      { url.push_str(&clause("country", "eq", &c)); }
        if filter.bear_community_member      { url.push_str(&clause("bear_community_member", "eq", "true")); }
        self.db.get_json::<Vec<Creator>>(&url).await
    }

    async fn find_by_id(&self, id: i64) -> Result<Option<Creator>, AppError> {
        let url = format!("{}/rest/v1/creators?select=*&id=eq.{}&limit=1", self.db.url, id);
