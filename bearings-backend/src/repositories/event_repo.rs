...
impl EventRepository for SupabaseEventRepository {
    async fn find(&self, filter: EventFilter) -> Result<Vec<Event>, AppError> {
        ...
        if let Some(m) = filter.month      { url.push_str(&clause("month",   "eq", &m)); }
        if let Some(t) = filter.event_type { url.push_str(&clause("type",    "eq", &t)); }
    ...
    async fn find_by_id(&self, id: i64) -> Result<Option<Event>, AppError> {
        let url = format!("{}/rest/v1/events?select=*&id=eq.{}&limit=1", self.db.url, id);
