...
pub struct SupabaseStoryRepository {
    db: SupabaseClient,
}
impl SupabaseStoryRepository {
...
impl StoryRepository for SupabaseStoryRepository {
    async fn find(&self, filter: StoryFilter) -> Result<Vec<Story>, AppError> {
        ...
        if let Some(t) = filter.story_type {
            url.push_str(&clause("story_type", "eq", &t));
        }
        if let Some(y) = filter.year_from {
            url.push_str(&clause("year", "gte", &y.to_string()));
        }
        if let Some(y) = filter.year_to {
            url.push_str(&clause("year", "lte", &y.to_string()));
        }
        if let Some(l) = filter.language_code {
            url.push_str(&clause("language_code", "eq", &l));
        }
        if filter.featured {
            url.push_str(&clause("featured", "eq", "true"));
        }
