...
pub trait VoteRepository: Send + Sync {
    ...
    async fn record_vote(&self, proposal_id: i64, voter_id: i64, vote: &str, weight: i32) -> Result<bool, AppError>;
    /// Set a proposal's status.
    async fn update_proposal_status(&self, proposal_id: i64, status: &str) -> Result<(), AppError>;
}

pub struct SupabaseVoteRepository { db: SupabaseClient }
impl SupabaseVoteRepository { pub fn new(db: SupabaseClient) -> Self { Self { db } } }
...
impl VoteRepository for SupabaseVoteRepository {
    ...
    async fn record_vote(&self, proposal_id: i64, voter_id: i64, vote: &str, weight: i32) -> Result<bool, AppError> {
        ...
        Ok(self.db.write_json_returning(reqwest::Method::POST, &url, &body).await.is_ok())
    }

    async fn update_proposal_status(&self, proposal_id: i64, status: &str) -> Result<(), AppError> {
        let url = format!("{}/rest/v1/bear_future_proposals?id=eq.{}", self.db.url, proposal_id);
        self.db.write_json(reqwest::Method::PATCH, &url, &serde_json::json!({ "status": status })).await
    }
}
