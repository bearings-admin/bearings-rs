...
pub trait PlaceRepository: Send + Sync {
    ...
    async fn find_nearby(&self, lat: f64, lng: f64, radius_km: f64)
...
impl PlaceRepository for SupabasePlaceRepository {
    async fn find(&self, filter: PlaceFilter) -> Result<Vec<Place>, AppError> {
        ...
        if let Some(t) = filter.place_type {
            url.push_str(&clause("place_type", "eq", &t));
        }
        if let Some(c) = filter.city {
            url.push_str(&clause("city", "eq", &c));
        }
        if filter.bear_popular {
            url.push_str(&clause("bear_popular", "eq", "true"));
        }
        if filter.men_only {
            url.push_str(&clause("men_only", "eq", "true"));
        }
    ...
    async fn find_by_id(&self, id: i64) -> Result<Option<Place>, AppError> {
        let url = format!(
            "{}/rest/v1/places?select=*&id=eq.{}&limit=1",
            self.db.url, id
        );
